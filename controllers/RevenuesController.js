...
class RevenuesController {
  ...
  static async updateRecipe(req, res) {
    const { id } = req.params;
    const { nome, rendimento, modoDePreparo, custoDeProducao } = req.body;
    ...
    try {
      ...
      const receitaAtualizada = await prisma.$transaction(async (tx) => {
        ...
        if (custoDeProducao !== undefined) {
          dataParaAtualizacao.custoDeProducao = parseFloat(custoDeProducao);
        } else {
          let custoCalculado = 0;

          const currentRecipeIngredients =
            await tx.tbReceitaIngrediente.findMany({
              where: { idReceita: id },
          ...
          for (const item of currentRecipeIngredients) {
            ...
            if (
              isNaN(custoDoItemNaReceita) ||
              !isFinite(custoDoItemNaReceita)
            ) {
              console.warn(
            ...
            custoCalculado += custoDoItemNaReceita;
          }

          dataParaAtualizacao.custoDeProducao = custoCalculado;
        }
